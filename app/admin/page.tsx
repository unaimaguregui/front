...
export default function PanelAdministracion() {
  ...
  const ejecutarProceso = async (endpoint: 'run_etl' | 'curar_bd', tipo: 'etl' | 'curar') => {
    ...
    setResultado(null);
    try {
      const res = await fetch(`http://127.0.0.1:8000/api/admin/${endpoint}`, { method: 'POST' });
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-[#1e293b] border border-slate-700 rounded-xl p-6 shadow-lg flex flex-col justify-between">
            <div>
              ...
              <p className="text-slate-400 text-sm mb-6 leading-relaxed">
                ...
                Lee todos los archivos, limpia errores y crea la base de datos DuckDB desde cero.
              </p>
            </div>
            <button
          ...
          <div className="bg-[#1e293b] border border-slate-700 rounded-xl p-6 shadow-lg flex flex-col justify-between">
            <div>
              ...
              <p className="text-slate-400 text-sm mb-6 leading-relaxed">
                <strong>¿Cuándo usarlo?</strong> Ejecútalo si cambias los diccionarios, los pesos o el algoritmo matemático.
                Recalcula todos los percentiles y ratings usando la base de datos existente (es más rápido).
              </p>
            </div>
            <button
        ...
        <div className="mt-8">
          {loading && (
            <div className="bg-slate-800 p-6 rounded-xl border border-slate-600 flex flex-col items-center justify-center text-center animate-pulse">
              ...
              <p className="text-white font-bold text-lg">
                {loading === 'etl' ? 'Leyendo y limpiando CSVs en el Servidor...' : 'Aplicando Campanas de Gauss y Z-Scores...'}
              </p>
              <p className="text-slate-400 text-sm mt-2 font-mono bg-slate-900 px-4 py-1 rounded-full border border-slate-700">
          ...
          {resultado && !loading && (
            <div className={`p-5 rounded-xl border flex items-start gap-3 ${
              ...
              {resultado.tipo === 'success' ? <CheckCircle2 className="mt-0.5 shrink-0" size={24} /> : <AlertCircle className="mt-0.5 shrink-0" size={24} />}
