...
export default function TeamFitPage() {
  ...
  useEffect(() => {
    async function cargarDatos() {
      try {
        const resEq = await fetch("http://localhost:8000/api/equipos_tacticos");
        ...
        const resCat = await fetch("http://localhost:8000/api/catalogo");
  ...
  const calcularTeamFit = async () => {
    ...
    try {
      const res = await fetch("http://localhost:8000/api/team_fit", {
